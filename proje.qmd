...
# Aylara göre özet istatistikler
aylik_ozet <- veri %>%
  group_by(Ay) %>%
  summarise(
    PM10_Ortalama = mean(PM10, na.rm = TRUE),
  ...
  arrange(match(Ay, month.abb))  # Ay sırasını korumak için

# Sonuçları görüntüle
print(aylik_ozet)

# Grafik için uzun form
aylik_grafik <- aylik_ozet %>%
  select(Ay, PM10_Ortalama, PM2_5_Ortalama) %>%
  pivot_longer(-Ay, names_to = "Parametre", values_to = "Ortalama")

ggplot(aylik_grafik, aes(x = Ay, y = Ortalama, fill = Parametre)) +
  geom_col(position = "dodge") +
  labs(
    title = "Aylara Göre Ortalama PM10 ve PM2.5 Değerleri",
    x = "Ay", y = "Ortalama (µg/m³)", fill = "Parametre"
  ) +
  scale_fill_manual(values = c("PM10_Ortalama" = "firebrick", "PM2_5_Ortalama" = "darkblue")) +
  theme_minimal()
```
