...
aylik_trend <- veri %>%
  ...
  group_by(Ay) %>%
  summarise(
    PM10_ort = mean(PM10, na.rm = TRUE),
    PM2_5_ort = mean(PM2_5, na.rm = TRUE)
  )

veri$Yil <- year(veri$Tarih)

# İlçe ve yıla göre yıllık ortalamaları hesapla
yillik_ortalama <- veri %>%
...
# PM10 Trend Grafiği (İlçe Yıllık Ortalama Bazlı)

ggplot(yillik_ortalama, aes(x = Yil, y = PM10_ort, color = Istasyon)) +
  geom_line(size = 1) +
  geom_point() +
  labs(title = "İlçelere Göre Yıllık PM10 Ortalaması - Trend Analizi",
       x = "Yıl", y = "PM10 (µg/m³)", color = "İstasyon") +
  theme_minimal()

# PM2.5 Trend Grafiği (İlçe Yıllık Ortalama Bazlı)

ggplot(yillik_ortalama, aes(x = Yil, y = PM2_5_ort, color = Istasyon)) +
  geom_line(size = 1) +
  geom_point() +
  labs(title = "İlçelere Göre Yıllık PM2.5 Ortalaması - Trend Analizi",
       x = "Yıl", y = "PM2.5 (µg/m³)", color = "İstasyon") +
  theme_minimal()
